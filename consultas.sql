...
SELECT DISTINCT d.nombre AS nombre_departamento FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor;
SELECT DISTINCT p.nombre AS nombre_alumno FROM persona p LEFT JOIN alumno_se_matricula_asignatura al ON p.id = al.id_alumno LEFT JOIN curso_escolar ce ON al.id_curso_escolar = ce.id WHERE ce.anyo_inicio = 2018 AND ce.anyo_fin = 2019;
...
SELECT p.nombre, p.apellido1, p.apellido2 FROM profesor pro LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor LEFT JOIN persona p ON pro.id_profesor = p.id WHERE a.id IS NULL;
...
SELECT DISTINCT d.nombre AS nombre_departamento FROM departamento d LEFT JOIN profesor pro ON d.id = pro.id_departamento LEFT JOIN asignatura a ON pro.id_profesor = a.id_profesor LEFT JOIN alumno_se_matricula_asignatura m ON a.id = m.id_asignatura WHERE m.id_asignatura IS NULL;
...
SELECT g.nombre AS nombre_grado, a.tipo AS tipo_asignatura, SUM(a.creditos) AS suma_creditos FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.id, g.nombre, a.tipo ORDER BY g.nombre ASC, a.tipo ASC;
